...
module.exports = class Orc {
  ...
  async addorcFile(request, response) {
    if (!request.files || !request.body.htmlTemplate) {
      return response.send({
        status: false,
        data: "required Contained Not Found",
      });
    }
    let data;
    try {
    ...
    let addFile = await orcService
      .addOcrByfile(data, request.userData)
      .catch((err) => {
        return { error: err };
      });
  ...
  async urlData(request, response) {
    ...
    let add = await orcService
      .addUrl(request.body.url, request.userData)
      .catch((err) => {
        return { error: err };
      });
    if (!add || add.error) {
  ...
  async orcListWithSearch(request, response) {
    let get = await orcService
      .orcListWithSerach(request.query, request.userData)
      .catch((err) => {
        return { error: err };
      });
    if (!get || get.error) {
